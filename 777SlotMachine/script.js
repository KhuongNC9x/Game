...
const wonCredit = (bet) => {
  const results = [
    111, 777, 333, 444, 555, 666, 888, 222, 11, 77, 33, 44, 55, 66, 88, 22,
  ];
  const awards = [
    1000, 800, 700, 60, 50, 40, 30, 20, 100, 20, 15, 10, 9, 8, 7, 5,
  ];
  ...
  const result =
    $(`#row_1 .SM_active img`).attr("key") +
    $(`#row_2 .SM_active img`).attr("key") +
    $(`#row_3 .SM_active img`).attr("key");
...
const spin = (min, max, spin, row, time) => {
  ...
  var activeValue = parseInt($(`#${row} .SM_active img`).attr("key")); // current value
  ...
  for (var i = 1; i <= pairs; i++) {
    for (var j = 1; j < max; j++) {
      ...
      $(`#${row}`).prepend(
        `<div class="SM_item"><img key='${activeValue}' src="image/${activeValue}.png" class="slotImage w-100" /></div>`
      );
    }
  }
  ...
  $(`#${row}`).css("top", `-${height - constWidth}px`); // set origin location
  ...
  setTimeout(() => {
    $(`#${row}`).html(
      `<div class="SM_item SM_active"><img key='${result}' src="image/${result}.png" class="slotImage w-100" /></div>`
    );
    $(`#${row}`).css({
      animation: "",
      top: 0,
